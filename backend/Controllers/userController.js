...
const setPassword = async (req, res) => {
    try {
        ...
        if (req.body.value.password === req.body.value.conPassword && req.body.value.password.trim().length !== 0) {
            
            const passwordHash = await securePassword(req.body.value.password)
         const pass=   await userModel.updateOne({ mobile: req.body.mobile}, { $set: { password: passwordHash } })
...
const getWorkerMoreData = async (req, res) => {
    try {
        ...
        const workerMore = await workerMoreDetailsModel.find()
        if (!workerMore) {
            return res.status(200).send({ message: 'worker data getting fail', success: false })
        }
        res.status(200).send({ message: 'worker data get', success: true, data: workerMore, category: categoryData })
    } catch (error) {
        res.status(500).send({ message: 'worker detail getting fail', success: false })
    }
}
const workerView = async (req, res) => {
    try {
        const workerMoreData = await workerMoreDetailsModel.findById(req.body.workerId)
        const ratingData = await ratingModel.findOne({ worker_id: workerMoreData.worker_id })
        if (!workerMoreData) {
            return res.status(200).send({ message: 'Not get workerdata', success: false })
        }
        res.status(200).send({ message: 'get worker data', success: true, data: workerMoreData,
         rating: ratingData
         })
    } catch (error) {
        res.status(500).send({ message: 'somthing went wrong', success: false })
...
const workerBooking = async (req, res) => {
    console.log("reached");
    try {
        ...
        const userbooking = await bookingModel.findOne({ worker_id: req.body.worker_id })
        ...
        if (dateValidation?.length > 0) {
            return res.status(200).send({ message: 'This worker not available', success: false })
        }
        const workerMoreData = await workerMoreDetailsModel.findOne({ worker_id: req.body.worker_id })
        if (userbooking) {
            await bookingModel.updateOne({ worker_id: req.body.worker_id },
                {
                    $push:
                    {
                        orders:
                        {
                            ...
                            category: workerMoreData.category,
                            image: workerMoreData.image,
                            fullAmount: workerMoreData.midBudjet
                        }
                    }
                })
            const findIndex = await bookingModel.findOne({worker_id: req.body.worker_id })
            const bookingSingleData = findIndex.orders[findIndex.orders.length - 1]
            await notificationModel.updateOne({ worker_id: req.body.worker_id, },
                {
                    $push:
                    {
                        notifications:
        ...
        const room = req.body.worker_id.concat(req.body.userId)
        const allNotificaions = await notificationModel.findOne({ worker_id: req.body.worker_id })
        console.log(allNotificaions);
...
const confirmBookingData = async (req, res) => {
    try {
        ...
        const categoryData = await workerMoreDetailsModel.findOne({ worker_id: filterdData[0].worker_id })
        if (!filterdData) {
...
const cancelBooking = async (req, res) => {
    try {
        ...
        if (dateObj > new Date) {
            ...
            await notificationModel.updateOne({ worker_id: bookData[0].worker_id },
                {
                    $push:
                    {
                        notifications:
...
const verifyFullPayment = async (req, res) => {
    try {
        ...
        const workeridData = await bookingModel.findOne({ _id: booked._id })
        if (hmac == details.payment.razorpay_signature) {
            await bookingModel.findOneAndUpdate({
                'orders._id': details.order.receipt
            },
            ...
            await notificationModel.updateOne(
                {
                    worker_id: workeridData.worker_id
                },
                {
                    $push: {
                        notifications: {
...
const allMedia = async (req, res) => {
    try {
        const mediaData = await mediaModel.findOne({ workerId: req.body.worker_id })
        res.status(200).send({
...
const reviewNotification = async (req, res) => {
    ...
    try {
        ...
        if (bookingData.length > 0) {
            for (const item of bookingData) {
                const data = await userNotificationModel.find({
                    user_id: req.body.userId,
                    'notifications.booking_id': item.orders._id,
                ...
                if (data.length === 0) {
                    await userNotificationModel.updateOne(
                        { user_id: req.body.userId },
                        {
                            $push: {
                                notifications: {
                                    name: `Hi ${item.orders.firstName}. Write a review about this worker.`,
                                    booking_id: item.orders._id,
                                    Actions: 'review',
                                    timestamp: new Date()
                                }
                            }
        ...
        res.status(200).send({ message: 'Review notifications updated successfully', success: true });
    } catch (error) {
        console.error(error);
...
const averageRating = async (worker_id) => {
    ...
    await ratingModel.updateOne({ worker_id: worker_id }, { $set: { average: averageFloat } })
    await workerMoreDetailsModel.updateOne({ worker_id: worker_id }, { $set: { averageRating: averageFloat } })
}
const writeReview = async (req, res) => {
    ...
    try {
        ...
        const ratingData = await ratingModel.findOne({ worker_id: bookingData[0].worker_id })
        if (bookingData) {
            if (!ratingData) {
                const ratings = new ratingModel({
                    worker_id: bookingData[0].worker_id,
                    reviews: [
                        {
                ...
                await ratingModel.updateOne({ worker_id: bookingData[0].worker_id },
                    {
                        $push:
                        {
                            reviews:
            ...
            await averageRating(bookingData[0].worker_id)
            // 
            await notificationModel.updateOne({ worker_id: bookingData[0].worker_id },
                {
                    $push:
                    {
                        notifications:
...
const chathistorys = async (req, res) => {
    try {
        ...
        { $project: { _id: 0, worker_id: 1, 'orders.payment_id': 1 } }])

        if (!bookingData || bookingData.length < 1) {
            return res.status(200).send({ message: 'Worker Booked Users only the chat option', success: false })
        }
...
const contact = async (req, res) => {
    try {
        ...
        for (let i = 0; i < req.body.length; i++) {
            const workerData = await workerMoreDetailsModel.findOne({
                worker_id: req.body[i].worker_id
            })
            workersData.push({
                ...workerData.toObject(),
        ...
        res.status(200).send({ message: 'contact get', success: true, data: workersData })
    } catch (error) {
